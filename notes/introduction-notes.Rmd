...
This is an R-Markdown document. It's a combination of text that is formatted using markdown syntax, R code, and results from running R code. It's saved as an `.Rmd` file. You can also write `.R` code that is analagous to a `.do` file where there's a list of commands with comments. 
