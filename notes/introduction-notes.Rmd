...
Behind the scenes, we are typing in "code-chunks" that get run by R. These code chunks are fenced in with the at the beginning 
and ending with . 
...
export PATH=/Applications/Stata/StataMP.app/Contents/MacOS/stata-mp:$PATH
```{r load-devtools}
library("devtools")
```
```{r setup-stata}
